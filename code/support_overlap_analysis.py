...
def main() -> None:
    ...
    binned = tmp.groupby("bin", observed=False).agg(R_sup_mean=("R_sup", "mean"), positive_fraction=("positive_margin", "mean"), n=("positive_margin", "size")).reset_index(drop=True)
    binned.to_csv(RESULTS / "support_overlap_positive_fraction.csv", index=False)
    plt.figure(figsize=(5.8, 3.7))
    width = 0.8 * np.diff(binned["R_sup_mean"]).mean() if len(binned) > 1 else 0.05
    plt.bar(binned["R_sup_mean"], binned["positive_fraction"], width=width, color="#59A14F", alpha=0.86)
    for _, row in binned.iterrows():
        plt.text(row["R_sup_mean"], min(1.02, row["positive_fraction"] + 0.035), f"n={int(row['n'])}", ha="center", va="bottom", fontsize=7, rotation=90)
    plt.xlabel(r"$R_{\rm sup}$")
    plt.ylabel("Empirical positive-margin fraction")
    plt.title("Support-overlap diagnostic")
    plt.ylim(-0.03, 1.03)
    plt.tight_layout()
    plt.savefig(FIGURES / "support_overlap_positive_fraction.pdf")
    plt.savefig(FIGURES / "support_overlap_positive_fraction.png", dpi=180)
    plt.close()
    summary = (
